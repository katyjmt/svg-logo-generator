...
// Use Validate Color package to validate text and shape colours
    ...
    run() {
        ...
        return Inquirer
            .prompt([
                ...
                {
                    type: 'input',
                    message: 'Please provide a name for your logo file.',
                    name: 'logoName',
                },
            ])
            // Once responses completed by user, generate shape and text SVG
            .then(({ text, textColour, shapes, shapeColour, logoName }) => {
                switch(shapes) {
                    case 'circle':
            ...
            .then(() => console.log('Your logo has been saved to the \'Examples\' directory.'))
            .catch((err) => {
                console.log(err);
